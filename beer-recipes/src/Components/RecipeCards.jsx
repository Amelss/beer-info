      ...
      <div className="bg-slate-100 rounded shadow-md pb-5 md:mx-96 ">
        {beerData.map((beers) => (
          <div key={id}>
            ...
            <h1 className="text-center text-1xl py-4 md:hidden">{beers.name}</h1>
            <img
              src={beers.image_url}
              alt="beer"
              ...
              />
              <div>
            <h1 className="hidden md:block text-3xl py-4 px-3">{beers.name}</h1>
            <p className="pt-5 text-xs px-3 md:text-sm">Tagline: {beers.tagline}</p>
            <p className="pt-3 text-xs text-gray-500 px-3 ">
              First Brew Date: {beers.first_brewed}
            </p>
            <p className="mt-1 text-sm px-3 font-light md:text-sm">{beers.description}</p>
